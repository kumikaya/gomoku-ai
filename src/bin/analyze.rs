use std::path::PathBuf;

use burn::{module::Module, store::ModuleRecord};
use clap::Parser;
use gomoku_ai::{
    game::analyze::analyze_game, inference::InferenceServer, network::transformer::GomokuNetwork,
};

#[derive(Parser)]
#[command(name = "gomoku-analyze")]
#[command(about = "五子棋 AI — 棋盘分析模式（自由落子 + MCTS 实时评估）")]
struct Cli {
    /// MCTS 模拟次数（默认 512）
    #[arg(short = 's', long, default_value = "512")]
    simulations: usize,

    /// 模型文件路径（.bpk）
    #[arg(short = 'm', long)]
    model_path: String,
}

fn main() {
    let cli = Cli::parse();

    let device = burn::tensor::Device::default();
    let path = PathBuf::from(&cli.model_path);

    let record = ModuleRecord::load(&path).unwrap_or_else(|e| {
        eprintln!("Error: failed to load model: {}", e);
        std::process::exit(1);
    });
    let model = GomokuNetwork::new(&device).load_record(record.cast_to_module_dtype());

    let server = InferenceServer::new(model, device.clone());
    println!("Model loaded. Starting analyze mode...");
    analyze_game(&server, cli.simulations);
}
